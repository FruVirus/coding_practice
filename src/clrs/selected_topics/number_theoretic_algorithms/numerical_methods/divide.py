...
Newton's method also requires high precision division when computing a / x_n. In
general, when we want a high precision representation of a / b and we will first compute
a high precision representation of 1 / b and then multiply by a. A high precision
...
The equation we want to solve for is:
    ...
    f'(x) = -1 / x^2
    x_(n + 1) = x_n - f(x_n) / f'(x_n) --> Newton's Method
    x_(n + 1) = x_n - (1 / x_n - b / R) / (-1 / x_n^2)
...
divide(): For multiplication and division operations, Theta(n^alpha), alpha >= 1
