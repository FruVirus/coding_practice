...
randomly selected person. In collaborative filtering, you find users similar to the
active user based on the intersection of their historical watches. You then collaborate
with similar users to generate candidate media for the active user.
...
You have a (n x m) matrix of user u_i (i = 1 to n) and movie m_j (j = 1 to m). Each
matrix element represents the feedback that the user i has given to a movie j. An empty
cell means that user i has not watched movie j. For example a 1 means that the media has
been watched/liked, a 0 means that the media has not been watched/disliked, and an empty
cell means that the media has no impressions yet.
...
nearest neighbour is weighted by their similarity with user i. The unseen movies with
good predicted feedback will be chosen as candidates for user i’s recommendations.
...
candidates for user A’s recommendation. The user-based approach is often harder to scale
as user preference tends to change over time. Items, in contrast, don’t change so the
item-based approach can usually be computed offline and served without frequent
re-training.

Method 2: Matrix factorization

You need to represent the user-media interaction matrix in a way that is scalable and
handles sparsity. Here, matrix factorization helps us by factoring this matrix into two
...
also be thought of as features of a movie or a user. User A's feedback for movie B is
captured in the form of alignment between A's preferences and B's characteristics.
...
by the length of the vectors. Finally, you multiply the movies’ normalized TF vectors
and the IDF vector element-wise to make TF-IDF vectors for the movies.

TF-IDF puts the similarity between documents into perspective by considering the rarity
of a feature throughout the corpus.
...
using past watches to recommend new ones, you can just compute the similarity (dot
product) between the user’s profile and media profiles of unseen movies to generate
relevant candidates for user A’s recommendations.
...
The actual feedback label will be positive when the media aligns with user preferences
and negative when it does not. To make the predicted feedback follow the same pattern,
the network learns the user and media embeddings in such a way that their distance will
be minimized if the user would like this media and maximized if the user would not like
the media.
...
Let’s assume that you have to generate two candidates for each user. After the user and
media vector embeddings have been generated, you will apply KNN and select candidates
for each user. It can be observed that user A’s nearest neighbors are movie B and movie
C, based on high vector similarity. Whereas, for user B, movie D and movie B are the
nearest neighbors.
...
this matrix into two lower dimensional matrices:
    ...
    Each user in the user profile matrix is represented by a row, which is a latent
vector of M dimensions.

    2. Media profile matrix (M x m)

    Each movie in the movie profile matrix is represented by a column, which is a latent
vector of M dimensions.
...
The representation of users and media in the form of a vector of latent factors aims to
explain the reason behind a user’s particular feedback for a media. Latent vectors can
also be thought of as features of a movie or a user. For example, in the diagram below,
two of the latent factors might loosely represent the amount of comedy and quirkiness.

Note that we say the word “might” and “loosely” for latent factors because we don’t know
exactly what each dimension means, it is “hidden” from us.
...
User A's feedback for movie B is captured in the form of alignment between A's
preferences and B's characteristics.
...
manually assigned media-descriptive-tags (e.g., visually striking, nostalgic, magical
creatures, character development, winter season, quirky indie rom-com set in Oregon,
etc.) by Netflix taggers. The media is represented as a vector of its attributes. The
following explanation takes a subset of the attributes for ease of understanding.
...
recommending media to the user:
    ...
    You can recommend movies to the user similar to those they have interacted (seen)
with in the past. This can be achieved by computing the dot product of movies.

    2. Similarity between media and user profiles

    The media’s TF-IDF vectors can be viewed as their profile. Based on user
preferences, which can be seen from its historical interactions with media, you can
build user profiles as well.

    Now, instead of using past watches to recommend new ones, you can just compute the
similarity (dot product) between the user’s profile and media profiles of unseen movies
to generate relevant candidates for user A’s recommendations.
...
Let’s look at some strengths and weaknesses of the approaches for candidate generation
discussed above.
