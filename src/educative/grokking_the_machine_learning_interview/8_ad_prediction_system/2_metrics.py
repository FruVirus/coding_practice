...
The metrics used in our ad prediction system should ensure that the machine learning
models help the overall improvement of the platform, increase revenue, and provide value
for the advertisers.
...
Offline metrics are mainly used to compare the models offline quickly and see which one
gives the best result. Online metrics are used to validate the model for an end-to-end
system to see how the revenue and engagement rate improve before making the final
decision to launch the model.
...
As we build models, the best way to compare them is to measure prediction accuracy
instead of measuring revenue impact directly.

Log Loss

Unlike AUC which looks at how well a model can classify a binary target, log loss
evaluates how close a model’s predicted values (uncalibrated probability estimates) are
to the actual target value. This metric captures to what degree expected probabilities
diverge from class labels. As such, it is an absolute measure of quality, which accounts
for generating well-calibrated, probabilistic output.
...
Let’s consider a scenario that differentiates why log loss gives a better output
compared to AUC. If we multiply all the predicted scores by a factor of 2 and our
average prediction rate is double than the empirical rate, AUC won’t change but log loss
will go down.
...
The metrics used in our ad prediction system will help select the best machine-learned
models to show relevant ads to the user. They should also ensure that these models help
the overall improvement of the platform, increase revenue, and provide value for the
advertisers.
...
When we have a significant class imbalance, i.e., the distribution is skewed towards
positive and negative class, we calibrate our model to estimate the likelihood of a data
...
As we build models, the best way to compare them is to measure prediction accuracy
instead of measuring revenue impact directly. The following are a few metrics that
enable us to compare the two models better offline.
...
system. It’s important to call out that just measuring revenue is a very short term
approach, as we may not provide enough value to advertisers and they will move away from
the system. However, revenue is definitely one critical metric to track. We will discuss
other essential metrics to track shortly.
...
Some of the actions might be:
    ...
    This will measure the action rate of a particular action targeted by the advertiser
e.g. add to cart rate, purchase rate, message rate etc.
...
All of the metrics discussed above can be used to measure user engagement with ad and
advertiser revenue on user engagement.
"""
