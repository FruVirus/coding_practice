...
User --> Generate Feed --> Tweet Selection --> Returned Candidates Tweets (I/O) -->
[Ranker --> Feed (I/O) --> Feed Engagement Data --> Training Data Generation -->
Ranker --> ...]

The Tweet Selection component performs the first step in generating a user’s feed, i.e.,
it fetches a pool of Tweets from the user’s network (the followees), since their last
login. This pool of Tweets is then forwarded to the ranker component.
...
engagement probabilities for display on user A’s feed. If we zoom into this component,
we can:
...
Let’s have a look at the architectural components that are integral in creating our
Twitter feed system.
