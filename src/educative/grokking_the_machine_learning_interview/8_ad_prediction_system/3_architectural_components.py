...
The query terms can be a partial match, full match, expansion, etc. For example,
whenever a user types a query “machine learning course”, the system shows the ML course
on educative.io.
    - User-based targeting: The ads will be subjective to the user based on a specific
region, demographic, gender, age, etc.
    - Interest-based targeting: This method shows interest-based ads. Assume that on
Facebook, the advertiser might want to show ads based on certain interest hierarchies.
For example, the advertiser might like to show sports-related ads to people interested
in sports.
...
more than ten minutes on the website. Here, we can expand our set and do seed audience
expansion.
...
The rank order is calculated based on predicted ad score (from the ad prediction
component) multiplied by the bid. Let’s call this the ad rank score.
...
CPE = Ad rank score of ad below / Ad rank score + 0.01
...
spread out throughout the day and the advertiser gets maximum return on investment (ROI)
on their campaign. This also prevents the overall system from having a significantly
high load at the start of the day, and the budget is spent evenly throughout the
campaign.
...
in terms of the number of ads in the system and the number of users on the platform. So,
it’s important to design the system in a way that it can scale well and be extremely
performance efficient without compromising on ads quality.
...
score. The ads selection component ranks the ads according to r, where
...
Let’s have a look at the high-level architecture of the system. There will be two main
actors involved in our ad prediction system - platform users and advertiser. Let’s see
...
Let’s briefly look at each component here.
...
The rank order is calculated based on predicted ad score (from the ad prediction
component) multipled by the bid. Let’s call this the ad rank score.
